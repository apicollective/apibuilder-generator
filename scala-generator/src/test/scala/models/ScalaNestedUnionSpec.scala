...
class ScalaNestedUnionSpec extends AnyFunSpec with Matchers {
  ...
  private val json: String = models.TestHelper.buildJson("""
      "imports": [],
      "headers": [],
      "info": [],
      "enums": [],
  ...
  it("generates valid inner type readers") {
    ...
    val code = Play2Json(ssd, scala3Support = false).readers(innerType)
  ...
  it("generates valid outer type readers") {
    ...
    val code = Play2Json(ssd, scala3Support = false).readers(outerType)
