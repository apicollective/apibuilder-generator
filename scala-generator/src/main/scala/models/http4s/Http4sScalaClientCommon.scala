...
object Http4sScalaClientCommon extends ScalaClientCommon {
  override def makeClientObject(
    ...
    s"""object Client {
        ...
        |    case ${http4sConfig.rightType}(value) => ${http4sConfig.asyncType}.${http4sConfig.asyncSuccess}(value)
        |    case ${http4sConfig.leftType}(error) => ${http4sConfig.asyncType}.${http4sConfig.asyncFailure}(new ${Namespaces(config.namespace).errors}.FailedRequest(r.${config.responseStatusMethod}, s"Invalid json for class[" + className + "]", None, error))
        |  }
        |}""".stripMargin.trim
  }
