...
object Http4sScalaClientCommonV0_23 extends ScalaClientCommon {
  ...
  override def makeClientObject(
    ...
    s"""object Client {
       ...
       |    className: String,
       |    r: ${config.responseClass}
       |  )(implicit entityDecoder: EntityDecoder[${http4sConfig.asyncType}, T]): ${http4sConfig.asyncType}[T] = r.attemptAs[T].${http4sConfig.monadTransformerInvoke}.flatMap {
       |    case ${http4sConfig.rightType}(value) => ${http4sConfig.wrappedAsyncType("Concurrent").getOrElse(http4sConfig.asyncType)}.pure(value)
       |    case ${http4sConfig.leftType}(error: DecodeFailure) => ${http4sConfig.wrappedAsyncType("Concurrent").getOrElse(http4sConfig.asyncType)}.${http4sConfig.asyncFailure}(new ${Namespaces(config.namespace).errors}.FailedRequest(r.${config.responseStatusMethod}, s"Invalid json for class[" + className + "]", None, error))
       |  }
       |}""".stripMargin.trim
  }
