...
import models.http4s018.{ScalaGeneratorUtil, ScalaService}
  ...
  def generate(): String = {
    val versionCapture = version.fold("") { ver =>
      s"""
         ...
         |  }
         |
         |  def apply(req: org.http4s.Message[IO]): Boolean = req.headers.get(ApiVersionMajor) match {
    ...
    val resources = resourcesAndRoutes.map { case (resource, routes) =>
      ...
      s"""trait $name {
         ...
         |
         |  implicit def circeJsonDecoder[A](implicit decoder: _root_.io.circe.Decoder[A]) = org.http4s.circe.jsonOf[IO, A]
         |  implicit def circeJsonEncoder[A](implicit encoder: _root_.io.circe.Encoder[A]) = org.http4s.circe.jsonEncoderOf[IO, A]
         |
         |${routes.map(_.operation().mkString("\n")).mkString("\n\n").indent(2)}
         |
         |  def apiVersionMatch(req: org.http4s.Message[IO]): Boolean = ApiVersion(req)
         |
         |  def service() = org.http4s.HttpService[IO] {
