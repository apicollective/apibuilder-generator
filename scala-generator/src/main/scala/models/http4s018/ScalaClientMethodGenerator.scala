  ...
  override def methods(resource: ScalaResource): Seq[ScalaClientMethod] = {
    resource.operations.map { op =>
      ...
      val defaultResponse = op.responses.find { r =>
        ...
        case Some(response) => {
          if (response.isUnit) {
            s"case r => IO.raiseError(new ${namespaces.errors}.${response.errorClassName}(r.${config.responseStatusMethod}))"
          } else {
            s"case r => IO.raiseError(new ${namespaces.errors}.${response.errorClassName}(r))"
          }
        }
        case None => {
          s"""case r => IO.raiseError(new ${namespaces.errors}.FailedRequest(r.${config.responseStatusMethod}, s"Unsupported response code[""" + "${r." + config.responseStatusMethod + s"""}]. Expected: ${allResponseCodes.mkString(", ")}"${Http4sScalaClientCommon.failedRequestUriParam(config)}))"""
        }
      }
            ...
            case ResponseCodeInt(statusCode) => {
              if (response.isSuccess) {
                if (featureMigration.hasImplicit404s && response.isOption) {
                  if (response.isUnit) {
                    Some(s"case r if r.${config.responseStatusMethod} == $statusCode => IO.pure(Some(()))")
                  } else {
                    val json = config.toJson("r", response.datatype.name)
                    Some(s"case r if r.${config.responseStatusMethod} == $statusCode => IO.pure(Some($json))")
                  }

                } else if (response.isUnit) {
                  Some(s"case r if r.${config.responseStatusMethod} == $statusCode => IO.pure(())")
                ...
                if (response.isUnit) {
                  Some(s"case r if r.${config.responseStatusMethod} == $statusCode => IO.raiseError(new ${namespaces.errors}.${response.errorClassName}(r.${config.responseStatusMethod}))")

                } else {
                  Some(s"case r if r.${config.responseStatusMethod} == $statusCode => IO.raiseError(new ${namespaces.errors}.${response.errorClassName}(r))")
                }
              }
            }
