...
case class Play2ClientGenerator(
  ...
  private def client(): String = {
    ...
    val patchMethod = version.supportsHttpPatch match {
      case true => """_logRequest("PATCH", _requestHolder(path).withHeaders(requestHeaders:_*).withQueryString(queryParameters:_*)).patch(body.getOrElse(play.api.libs.json.Json.obj()))"""
...
${methodGenerator.objects().indent(4)}
    ...
    def _requestHolder(path: String): ${version.requestHolderClass} = {
      ${if (!version.config.expectsInjectedWsClient) "import play.api.Play.current\n\n"}
